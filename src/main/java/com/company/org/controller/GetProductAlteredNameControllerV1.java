...
public class GetProductAlteredNameControllerV1 {
    ...
    @Autowired
    GetProductAlteredNameServiceV1 getProductAlteredNameServiceV1;
    ...
    @ResponseBody
    @GetMapping(
        path = "/retail/product/altered-name/{id}",
        produces = { MediaType.APPLICATION_JSON_VALUE }
    )
    @Operation(
        summary = "Get Product Record With Altered Name",
        description = "Performs a GET operation to retrieve a product record based on id and does a lookup for an altered name",
        tags = "MyRetailAPI",
        parameters = {
            @Parameter(
                name = "id",
    ...
    @ApiResponses(value = {
        @ApiResponse(
            ...
            examples = @ExampleObject(value = "{ \"_id\": 13860428, \"name\": \"The Big Lebowski (Blu-ray)\", \"current_price\": { \"value\": 13.49, \"currency_code\": \"USD\" } }"))
        ),
        @ApiResponse(
        ...
        @ApiResponse(
            ...
            content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
            schema = @Schema(implementation = ErrorResponse.class),
            examples = @ExampleObject(value = "{ \"timeStamp\": \"Sat Mar 21 17:00:00 GMT 2020\", \"status\": 409, \"error\": \"Not Found\", \"message\": \"Altered name record is not valid JSON\" }"))
        ),
        @ApiResponse(
    ...
    public ResponseEntity<String> getProductAlteredNameById(@PathVariable(name = "id") String id,
        ...
        ResponseVO response = getProductAlteredNameServiceV1.doService(requestVO);
